...
def fetch_data(producer, rawdata):
	...
	for data_object in data["bike-location"]:
		try:
			logger.debug('received bike location data')
			data_string = json.dumps(data_object)
			producer.send(topic_name, data_string)
		except KafkaTimeoutError as timeout_error:
			logger.warn('time out error failed to send bike location data to kafka')
		except Exception:
			logger.warn('failed to send bike location data')
		time.sleep(5)
